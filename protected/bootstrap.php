 ...
 */
defined('YII_ENABLE_EXCEPTION_HANDLER') or define('YII_ENABLE_EXCEPTION_HANDLER',
                true);
/**
 ...
 */
defined('YII_PATH') or define('YII_PATH',
...
class YiiBase
{
    ...
    public static function setApplication($app)
    {
        ...
        else throw new CException(Yii::t('yii',
    ...
    public static function createComponent($config)
    {
        ...
        foreach ($config as $key => $value) $object->$key = $value;
    ...
    public static function getPathOfAlias($alias)
    {
        ...
        elseif (($pos = strpos($alias, '.')) !== false)
        {
            ...
            if (isset(self::$_aliases[$rootAlias])) return self::$_aliases[$alias]
                        = rtrim(self::$_aliases[$rootAlias] . DIRECTORY_SEPARATOR . str_replace('.',
                                DIRECTORY_SEPARATOR, substr($alias, $pos + 1)),
                        '*' . DIRECTORY_SEPARATOR);
            ...
            $category = 'application')
    {
        ...
        if (YII_DEBUG && YII_TRACE_LEVEL > 0 && $level !== CLogger::LEVEL_PROFILE)
        {
            ...
            {
                if (isset($trace['file'], $trace['line']) && strpos($trace['file'],
                                YII_PATH) !== 0)
                {
...
), $source = null, $language = null)
    {
        if (self::$_app !== null)
        {
            if ($source === null) $source = ($category === 'yii' || $category === 'zii')
                            ? 'coreMessages' : 'messages';
            if (($source = self::$_app->getComponent($source)) !== null) $message
                        = $source->translate($category, $message, $language);
        }
        if ($params === array(
)) return $message;
