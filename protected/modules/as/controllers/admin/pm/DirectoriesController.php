...
class DirectoriesController extends AdminController
{
	...
	public function actionIndex()
	{
		$this->breadcrumbs = array(
			array('Pm Directories', array('index')),
			array('Overview', '', array('class' => 'active'))
		);
		
		if((($access = AccessControl::GetAccess('PmDirectory::Overview')) === false) || $access->read != 1)
			$this->denyAccess();
		...
		$count=PmDirectory::model()->count($criteria);
		$pages=new CPagination($count);

		// results per page
		$pages->pageSize = isset($_GET['per-page']) ? (int)$_GET['per-page'] : 10;
		$pages->applyLimit($criteria);
		$models = PmDirectory::model()->findAll($criteria);
		
		$this->render('_main_view',array(
			'models' => $models,
			'pages' => $pages,
			'form_model' => new PmDirectory,
			'can' => $access
		));
	}
	...
	{
		if((($access = AccessControl::GetAccess('PmDirectory::Overview')) === false) || $access->delete != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('PmDirectory::Overview')) === false) || $access->write != 1)
			$this->denyAccess();
		...
		$model=new PmDirectory;

		// comment the following line if AJAX validation is unneeded
		$this->performAjaxValidation($model);

		if(isset($_POST['PmDirectory']))
		{
			$model->attributes=$_POST['PmDirectory'];
			if($model->save())
	...
	{
		if((($access = AccessControl::GetAccess('PmDirectory::Overview')) === false) || $access->update != 1)
			$this->denyAccess();
	...
	{
		$model=PmDirectory::model()->findByPk($id);
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='pm-directory-form')
		{
