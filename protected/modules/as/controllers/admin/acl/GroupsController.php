...
class GroupsController extends AdminController
{
	...
	public function actionIndex()
	{
		$this->breadcrumbs = array(
			array('Acl Groups', array('index')),
			array('Overview', '', array('class' => 'active'))
		);
		
		if((($access = AccessControl::GetAccess('AclGroup::Overview')) === false) || $access->read != 1)
			$this->denyAccess();
		...
		$count=AclGroup::model()->count($criteria);
		$pages=new CPagination($count);

		// results per page
		$pages->pageSize = isset($_GET['per-page']) ? (int)$_GET['per-page'] : 10;
		$pages->applyLimit($criteria);
		$models = AclGroup::model()->findAll($criteria);
		
		$this->render('_main_view',array(
			'models' => $models,
			'pages' => $pages,
			'form_model' => new AclGroup,
			'can' => $access
		));
	}
	...
	{
		if((($access = AccessControl::GetAccess('AclGroup::Overview')) === false) || $access->delete != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('AclGroup::Overview')) === false) || $access->write != 1)
			$this->denyAccess();
		...
		{
			$model->attributes=$_POST['AclGroup'];
			if($model->save())
	...
	{
		if((($access = AccessControl::GetAccess('AclGroup::Overview')) === false) || $access->update != 1)
			$this->denyAccess();
	...
	{
		$model=AclGroup::model()->findByPk($id);
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='acl-group-form')
		{
