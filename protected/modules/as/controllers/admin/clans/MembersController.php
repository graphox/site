...
class MembersController extends AdminController
{
	...
	public function actionIndex()
	{
		$this->breadcrumbs = array(
			array('Clan Members', array('index')),
			array('Overview', '', array('class' => 'active'))
		);
		
		if((($access = AccessControl::GetAccess('ClanMembers::Overview')) === false) || $access->read != 1)
			$this->denyAccess();
		...
		if(isset($_POST['search']) && is_array($_POST['search']))
		{
				...
				if(isset($_POST['search']['rank_id']))
					$criteria->compare('rank_id', $_POST['search']['rank_id']);
		}
		else
		...
		$count=ClanMembers::model()->count($criteria);
		$pages=new CPagination($count);

		// results per page
		$pages->pageSize = isset($_GET['per-page']) ? (int)$_GET['per-page'] : 10;
		$pages->applyLimit($criteria);
		$models = ClanMembers::model()->findAll($criteria);
		
		$this->render('_main_view',array(
			'models' => $models,
			'pages' => $pages,
			'form_model' => new ClanMembers,
			'can' => $access
		));
	}
	...
	{
		if((($access = AccessControl::GetAccess('ClanMembers::Overview')) === false) || $access->delete != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('ClanMembers::Overview')) === false) || $access->write != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('ClanMembers::Overview')) === false) || $access->update != 1)
			$this->denyAccess();
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='clan-members-form')
		{
