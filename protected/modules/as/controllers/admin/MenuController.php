...
class MenuController extends AdminController
{
	...
	public function actionIndex()
	{
		$this->breadcrumbs = array(
			array('Menu Items', array('index')),
			array('Overview', '', array('class' => 'active'))
		);
		
		if((($access = AccessControl::GetAccess('MenuItem::Overview')) === false) || $access->read != 1)
			$this->denyAccess();
		...
		if(isset($_POST['search']) && is_array($_POST['search']))
		{
				...
				if(isset($_POST['search']['url']))
					$criteria->compare('url', $_POST['search']['url']);
		...
		$count=MenuItem::model()->count($criteria);
		$pages=new CPagination($count);

		// results per page
		$pages->pageSize = isset($_GET['per-page']) ? (int)$_GET['per-page'] : 10;
		$pages->applyLimit($criteria);
		$models = MenuItem::model()->findAll($criteria);
		
		$this->render('_main_view',array(
			'models' => $models,
			'pages' => $pages,
			'form_model' => new MenuItem,
			'can' => $access
		));
	}
	...
	{
		if((($access = AccessControl::GetAccess('MenuItem::Overview')) === false) || $access->delete != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('MenuItem::Overview')) === false) || $access->write != 1)
			$this->denyAccess();
	...
	{
		if((($access = AccessControl::GetAccess('MenuItem::Overview')) === false) || $access->update != 1)
			$this->denyAccess();
