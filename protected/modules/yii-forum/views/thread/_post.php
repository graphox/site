...
<div class="post">
    <div class="header">
        <?php echo Yii::app()->controller->module->format_date($data->created, 'long'); ?> by <?php echo CHtml::link(CHtml::encode($data->author->displayname), $data->author->url); ?>
        <?php if($data->editor) echo ' (Modified: '. Yii::app()->controller->module->format_date($data->updated, 'long') .' by '. CHtml::link(CHtml::encode($data->editor->displayName), $data->editor->url) .')'; ?>
        <?php
            if($isAdmin)
            {
    ...
    <?php if($isAdmin || Yii::app()->user->id == $data->author->id): ?>
        <div class="footer">
