...
$newthread = $forum->isLocked?'':'<div class="newthread" style="float:right;">'. CHtml::link(CHtml::image(Yii::app()->controller->module->registerImage("newthread.gif")), array('/forum/thread/create', 'id'=>$forum->id)) .'</div>';
...
$gridColumns = array(
    array(
        ...
        'type' => 'html',
        'value' => 'CHtml::image(Yii::app()->controller->module->registerImage("folder". ($data->isLocked?"locked":"") .".gif"), ($data->isLocked?"Locked":"Unlocked"), array("title"=>$data->isLocked?"Thread locked":"Thread unlocked"))',
...
$this->widget('forum.extensions.groupgridview.GroupGridView', array(
    ...
    'extraRowColumns' => array('isSticky'),
    'extraRowExpression' => '"<b>".($data->isSticky?"Sticky threads":"Normal threads")."</b>"',
    'columns' => $gridColumns,
    'htmlOptions'=>array(
